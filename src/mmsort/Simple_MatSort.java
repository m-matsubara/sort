/*
 * MatSort (Simple implementation for education)
...
public class Simple_MatSort implements ISortAlgorithm {
	/**
	 * Forward priority binary search
	 * 前方優先バイナリサーチ
	 *
	...
	public static <T> void matSort(final T[] array, final int from, final int to, final Comparator<? super T> comparator, int workSize)
	{
		...
		Simple_MasSort.masSort(array, fromIdx, to, works, comparator);
		...
		while (fromIdx > from) {
			...
			if (fromIdx < from)
				fromIdx = from;
			Simple_MasSort.masSort(array, fromIdx, midIdx, works, comparator);
...
//				T value2 = array[idx2];
	...
	{
		matSort(array, from, to, comparator, 10);
	}
	...
	{
		return "MatSort (Simple implementation)";
	}
}
